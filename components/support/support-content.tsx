...
export default function SupportContent({ user, initialActiveTab = 'contact' }: SupportContentProps) {
    ...
    useEffect(() => {
        ...
        if (tabFromUrl) {
            // If admin and URL somehow has contact/faq, initialActiveTab should have been 'tickets'
            // and page.tsx should have redirected.
            if (isAdmin && (tabFromUrl === 'contact' || tabFromUrl === 'faq')) {
                newActiveTab = 'tickets'; // Correct based on admin role
            } else {
                newActiveTab = tabFromUrl; // Sync with URL
            }
        } else {
            // No tab in URL, rely on initialActiveTab (which is role-aware from page.tsx)
            newActiveTab = initialActiveTab;
        }

        if (newActiveTab !== activeTab) {
            setActiveTab(newActiveTab);
        }
    }, [searchParams, initialActiveTab, isAdmin, activeTab]);
                    ...
                    <div className='space-y-6'>
                        ...
                        {user && (
                            <TabsContent value='tickets' className='mt-0'>
                                <TicketHistory
                                    onSwitchToContactTab={!isAdmin ? () => handleTabChange('contact') : undefined}
                                    isAdmin={isAdmin}
                                />
                            </TabsContent>
                        )}
